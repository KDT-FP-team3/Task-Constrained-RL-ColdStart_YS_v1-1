...
## 개선 이력 (improve 5-1 기준)

| 단계 | 파일 | 내용 |
|------|------|------|
| P0 | `base_agent.py` | 매직넘버 → 상수 (TRAIN_RATIO/ENTROPY_COEFF/Q_FLOOR_MARGIN) |
| P1 | `app.py` | Softmax Temperature 슬라이더 + Weight Cap (Fund Settings 사이드바) |
| P2 | `base_agent.py`, `app.py` | theta/Q 반환 + State Analysis Dashboard (Explainable RL) |
| P3 | `base_agent.py`, `data_loader.py` | `_encode_state` 선형 조합 명시화 + Rolling_Std + 8상태 변동성 모드 |
| P4 | `base_agent.py`, `app.py` | Rolling Window 재학습 (OOS 주기 재학습, STATIC RL 전용) |
...
## 핵심 상수 블록 (base_agent.py 최상단)

```python
TRAIN_RATIO       = 0.7    # 워크포워드 분할 비율
...
```
state = Σ(signal_i × 2^i)   ←  _encode_state(signals)

4상태 (기본): [is_bull, is_above_ema]          → state ∈ {0,1,2,3}
8상태 (P3):  [is_bull, is_above_ema, is_high_vol] → state ∈ {0..7}
```

## 신규 반환값 (improve 5-1)

`get_rl_data(...)` → 9개 반환 (기존 7개 + s_theta, v_qtable):
```
df, v_trace, s_trace, real_ret_trace, s_mdd, v_log, s_log, s_theta, v_qtable
```
모든 호출 지점: 9개 언패킹 필수 (_, _, ..., _, _)

## 신규 session_state 키 (improve 5-1)

```
fund_temperature   float    Softmax 온도 (1.0~5.0)
fund_max_weight    float    단일 종목 최대 비중 (0.1~1.0)
policy_cache       dict     {hist_key: {theta, q_table, n_states}}
use_vol_feature    bool     8상태 변동성 모드 활성화
roll_period_active bool     Rolling Window 재학습 활성화
