...
── On-policy (버퍼 없음) ──────────────┬── On+Off-policy ──┬── Off-policy (버퍼 있음) ──
  STATIC → A2C → A3C → PPO            │      ACER          │    SAC → DDPG
  Tabular  1-step n-step Clip+GAE      │  Retrace+IS(이산)  │  Entropy(이산)  Det.(연속)
```

| 순서 | 알고리즘 | 핵심 특징 | 행동 공간 | 버퍼 |
|-----|---------|---------|---------|------|
| 1 | **STATIC** | Tabular Actor-Critic, 4-상태 EMA×추세 이산화 | 이산 | ❌ |
...
| 항목 | STATIC | Vanilla | A2C | A3C | PPO | ACER | SAC | DDPG |
|------|--------|---------|-----|-----|-----|------|-----|------|
| 알고리즘 | PG Actor-Critic (Tabular) | Q-Learning | AC (신경망) | AC n-step | Clipped Surrogate | Retrace+IS | Soft AC | Deterministic PG |
| 상태 표현 | 4개 이산 | 2개 이산 | 5차원 연속 | 5차원 연속 | 5차원 연속 | 5차원 연속 | 5차원 연속 | 5차원 연속 |
| Q 추정 | TD V(s) | max Q | 1-step TD | n-step | GAE λ=0.95 | **Retrace(λ)** | Soft-V | Bellman |
| 탐험 | ε-greedy | ε annealing | 확률적 π | 확률적 π | 확률적 π | ε-greedy+IS | 자동 α | OU noise |
| 행동 공간 | 이산 | 이산 | 이산 | 이산 | 이산 | 이산 | 이산 | **연속** |
| 버퍼 | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ |
| 역할 | 주 평가 대상 | 비교 기준선 | 신경망 비교 | 신경망 비교 | 신경망 비교 | 신경망 비교 | 신경망 비교 | 신경망 비교 |
...
| 알고리즘 | 데이터 요구량 | 변동성 환경 | 안정성 | 해석 가능성 |
|---------|------------|-----------|-------|-----------|
| STATIC | 낮음 | 보통 | 높음 | 높음 (State Analysis) |
| Vanilla | 낮음 | 낮음 | 중간 | 높음 |
| A2C | 중간 | 중간 | 중간 | 낮음 |
| A3C | 중간 | 높음 | 중간 | 낮음 |
| PPO | 중간 | 중간 | 매우 높음 | 낮음 |
...
> STATIC은 시장을 단순화하여 해석 가능성이 높고, 신경망 알고리즘들은 더 복잡한 패턴을 학습하지만 파라미터와 데이터 양에 더 민감하다. 종목의 변동성과 데이터 길이에 따라 적합한 알고리즘이 달라진다.
