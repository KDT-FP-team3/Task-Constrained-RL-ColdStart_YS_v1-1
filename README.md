...
복수 시드 평가 결과를 단순 평균하는 대신, **시드 분산 패널티**와 **MDD 패널티**를 차감하여 안정적인 파라미터 집합을 선호하도록 유도한다.
...
# 복수 시드 평가
_gaps     = [composite_gap(seed_i) for seed_i in eval_seeds]
_mdd_list = [abs(s_mdd_i)          for seed_i in eval_seeds]

gap_mean = mean(_gaps)
gap_std  = std(_gaps)                              # 시드 간 분산
avg_mdd  = mean(_mdd_list)                         # 평균 최대낙폭

# 패널티 항
pen_std  = _LAMBDA_STD × gap_std                   # 불안정 파라미터 억제
pen_mdd  = _LAMBDA_MDD × max(0, avg_mdd − 15.0)   # 과도한 MDD 억제

# 최종 최적화 목표 (penalized gap)
gap_adj  = gap_mean − pen_std − pen_mdd
```

| 패널티 | 역할 | 효과 |
|--------|------|------|
| `pen_std` | 시드마다 결과가 크게 달라지는 파라미터 억제 | 재현성 높은 안정적 파라미터 선호 |
| `pen_mdd` | MDD > 15% 초과분 패널티 | 고수익이더라도 MDD 과대 파라미터 억제 |

> 수렴 차트의 "Alpha vs Market" 선은 `gap_mean`(패널티 미적용 원본)을 표시한다. 파라미터 저장 판단은 `gap_adj`로 이루어진다.
...
7. σ_max Cosine 상한 (Cosine Annealing Ceiling):
   σ_max_t = σ_min + 0.5 × (σ_max − σ_min) × (1 + cos(π × t / T_max))
   σ       = min(σ_reactive, σ_max_t)     ← 반응형 σ가 Cosine 상한 초과 불가
   t=0   : σ_max_t = σ_max = 0.30  (초기 최대 탐험)
   t=T/2 : σ_max_t ≈ 0.195         (중간 수렴)
   t=T   : σ_max_t = σ_min         (최종 정밀 수렴)
```

**Cosine 상한의 효과**: 후반 iteration에서 반응형 확장(`σ×1.04`)이 반복되어도 구조적으로 탐험 범위가 좁아진다. 반응형 적응 지능은 보존하면서 전체 수렴 방향을 보장한다.
...
STEP 3: _n_eval = min(4, max(3, auto_runs//2)),  eval_seeds = [seed + j*37]
  ...
  │    • s_mdd 수집 (각 시드별 최대낙폭)                              │
  │  STEP 7: gap_adj = mean(gaps) − 0.5×std(gaps) − 0.3×max(0,MDD-15%)│
  │          (수렴 차트 표시는 mean(gaps) 원본 사용)                  │
  │  STEP 8: V += value_alpha × (gap_adj − V)         (Critic EMA)   │
  │  STEP 9: A_norm = tanh((expected_gap − V) / 10)                  │
  │  STEP 10: pg_dir = Δ/σ (L2≤1);  μ += lr_actor × A_norm × pg_dir │
  │  STEP 11: A_norm > 0 → σ×0.96;  A_norm ≤ 0 → σ×1.04  (반응형)  │
  │           σ_max_t Cosine 상한 적용 → σ = min(σ, σ_max_t)        │
  │  STEP 12: best 갱신, 수렴 차트 실시간 업데이트                    │
  │           진행바 subtitle: σ=현재값 ↑Cosine상한, σ-pen, MDD-pen  │
  └────────────────────────────────────────────────────────────────────┘
      │
...
### improve 7-3 (현재) — Gap 패널티 조정 + Cosine σ_max 상한 + 차트 개선

| # | 변경 내용 |
|---|---------|
| P1 | `app.py` Gap 패널티 조정: `gap_adj = gap_mean − 0.5×std(gaps) − 0.3×max(0, avg_mdd−15%)` 도입 |
| P2 | `app.py` 시드별 `s_mdd` 수집 → `_mdd_list` 구성 → MDD 패널티 산출 |
| P3 | `app.py` 수렴 차트 subtitle에 σ-pen, MDD-pen 값 amber 색상 표시 |
| S1 | `common/heuristic.py` `PGActorCriticOptimizer.__init__`: `T_max` 파라미터 추가 |
| S2 | `common/heuristic.py` `update()`: σ_max_t Cosine 상한 계산 + `σ = min(σ_reactive, σ_max_t)` 적용 |
| S3 | `app.py` `PGActorCriticOptimizer` 호출에 `T_max=n_iters` 전달 |
| U1 | 차트 개선: State Policy Analysis expander `expanded=True` (항상 펼침) |
| U2 | Capital 비교 차트: Capital($) Y축, 우승자 배지, vs Market alpha 주석 |
| U3 | State Policy Analysis 차트 및 Capital 비교 차트 폰트 크기 전반 확대 |

---

### improve 7-2 — STATIC-H 하이브리드 알고리즘 + 차트 폰트 개선
